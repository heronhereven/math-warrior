...
def build_admin_app(port: int = 0, desktop_bridge=None) -> MathQuestApp:
...
class AdminLauncher:
    kind = "admin"
    title = "小和工作台桌面版"
    action_label = "打开小和工作台"
    copy = "双击后会启动小和工作台。第一次使用时，把同步仓库信息填进去保存就行。"
    ...
    def _setup_sync(self) -> None:
        ...
        self.sync_status = "GitHub 审核通道已启动，正在后台轮询。"
    ...
    def desktop_trigger_sync(self) -> dict:
        ...
        if self.sync_worker is None:
            self._setup_sync()
        if self.sync_worker is not None:
            self.sync_status = "正在手动触发一次拉取/回写..."
    ...
    def start(self) -> int:
        if self._reopen_existing():
            return 0
        app = build_admin_app(port=0, desktop_bridge=self)
        ...
        self.status_message = "小和工作台已经准备好了。可以先在控制台里保存同步设置，再打开工作台。"
...
def smoke_test() -> int:
    ...
    app = build_admin_app(port=0, desktop_bridge=launcher)
